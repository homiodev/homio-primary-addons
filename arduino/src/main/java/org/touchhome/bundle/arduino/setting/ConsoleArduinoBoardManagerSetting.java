...
public class ConsoleArduinoBoardManagerSetting implements SettingPluginPackageInstall, ConsoleSettingPlugin<JSONObject> {
    ...
    @Override
    public void unInstallPackage(EntityContext entityContext, PackageRequest packageRequest, String progressKey) throws Exception {
    ...
    private ContributedPlatform searchContributedPlatform(String name, String version) {
        ...
        throw new UserException("Unable to find board: " + name + " with version: " + version);
    }
    ...
    private ContributedPlatformReleases getContributedPlatformReleases(String name) {
        ...
        if (release == null) {
            throw new UserException("Unable to find board with name: " + name);
        }
        return release;
    }
