 ...
 * <p>
 * The configuration handler provides configuration services for a cluster. The handler supports the discovery of configuration attributes within the remote device, and the
 * generation of the ESH configuration descriptions. It is then able to process configuration updates, and sends the attribute updates to the device. It provides a getter for any
 * local configuration parameters (ie those that are not attributes in the remote device).
 */
