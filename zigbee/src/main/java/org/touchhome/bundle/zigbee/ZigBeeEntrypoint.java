...
public class ZigBeeEntrypoint implements BundleEntrypoint {
    ...
    public void init() {
        ...
        entityContext.setting().listenValue(
            ZigBeeEntityCompactModeSetting.class,
            "zigbee-compact-mode",
            (value) -> {
                entityContext.ui().updateItems(Z2MDeviceEntity.class);
                entityContext.ui().updateItems(ZigBeeDeviceEntity.class);
            });
        ...
        entityContext.event().addPortChangeStatusListener("zigbee-ports",
            port -> {
                ...
                testCoordinators(entityContext.findAll(Z2MLocalCoordinatorEntity.class), ports, coordinator -> coordinator.getService().restartCoordinator());
