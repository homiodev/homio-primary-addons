...
public abstract class ZigBeeBaseChannelConverter {
  ...
  /**
   * Configures the device. This method should perform the one off device configuration such as
   * performing the bind and reporting configuration.
   *
   ...
   * initialization of any internal fields of the converter other than those initialized in the
   * #initialize method.
   */
  public abstract void initializeDevice() throws Exception;

  /**
   * Initialize the converter. This is called by the {@link ZigBeeDeviceService} when the channel is
   * created. The converter should initialize any internal states, open any clusters, add reporting
   * and binding that it needs to operate.
   *
   * <p>
   */
  ...
  /**
   * Check if this converter supports features from the {@link ZigBeeEndpoint} If the converter
   * doesn't support any features, it returns null.
   *
   ...
   */
  public abstract boolean acceptEndpoint(
    ...
    if (discoverAttribute) {
      try {
        if (!cluster
                ...
                .get(getDiscoveryTimeout(entityContext), TimeUnit.SECONDS)
  ...
  /**
   * Sets the {@code pollingPeriod} and {@code maxReportingPeriod} depending on the success or
   * failure of the given reporting response.
   *
   * @param reportResponse a {@link CommandResult} representing the response to a reporting request
   * @param reportingFailedPollingInterval the polling interval to be used in case configuring
   *     reporting has failed
   * @param reportingSuccessMaxReportInterval the maximum reporting interval in case reporting is
  ...
  /**
   * Creates a binding from the remote cluster to the local {@link
   * ZigBeeProfileType#ZIGBEE_HOME_AUTOMATION} endpoint
   *
  ...
  // Configure reporting
  protected void updateDeviceReporting(
      @NotNull ZclCluster serverCluster,
      int attributeId,
      @NotNull ZclReportingConfig configReporting) {
    try {
      CommandResult reportingResponse =
          serverCluster
              .setReporting(
                  ...
                  configReporting.getReportingChange())
              .get();
      handleReportingResponse(
          reportingResponse,
