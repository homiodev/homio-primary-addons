...
public final class DeviceConfigurations {
        ...
        try {
            var objectNode =
                    CommonUtils.readAndMergeJSON(
                            "device-definitions.json", OBJECT_MAPPER.createObjectNode());
            for (JsonNode vendorNode : objectNode) {
                for (JsonNode deviceNode : vendorNode.get("devices")) {
                    ...
                    deviceDefinition.setVendor(
                            deviceNode.has("vendor")
                                    ? deviceNode.get("vendor").asText()
                                    : vendorNode.asText());
                    deviceDefinition.setImage(
                            ofNullable(deviceNode.get("image"))
                                    .map(JsonNode::textValue)
                                    .orElse(null));
                    deviceDefinition.setIcon(
                            ofNullable(deviceNode.get("icon"))
                                    .map(JsonNode::textValue)
                                    .orElseThrow(() -> noKeyFound(deviceNode, "icon")));
                    deviceDefinition.setIconColor(
                            ofNullable(deviceNode.get("iconColor"))
                                    .map(JsonNode::textValue)
                                    .orElseThrow(() -> noKeyFound(deviceNode, "iconColor")));
                    deviceDefinition.setCategory(
                            ofNullable(deviceNode.get("category"))
                                    .map(JsonNode::textValue)
                                    .orElse(null));
                    for (JsonNode endpoint : deviceNode.path("endpoints")) {
                        var endpointDefinition = new EndpointDefinition();
                        endpointDefinition.setId(
                                ofNullable(endpoint.get("id"))
                                        .map(JsonNode::textValue)
                                        .orElse(null));
                        endpointDefinition.setInputClusters(
                                JsonReaderUtil.getOptIntegerArray(endpoint, "input_clusters"));
                        endpointDefinition.setTypeId(endpoint.get("type_id").textValue());
                        if (endpoint.has("unit")) {
                            endpointDefinition.setUnit(
                                    Units.findUnit(endpoint.get("unit").asText()));
                        }
    ...
    private static Exception noKeyFound(JsonNode deviceNode, String key) {
        return new IllegalStateException(
                "Unable to find key: " + key + " in device: " + deviceNode);
    }

    public static Optional<DeviceConfiguration> getDeviceDefinition(
