...
import org.touchhome.bundle.zigbee.converter.command.TuyaButtonPressCommand;
...
public class ZigBeeConverterTuyaButton extends ZigBeeInputBaseConverter implements ZclCommandListener {
  ...
  public void initializeDevice() throws Exception {
    ZclCluster clientCluster = this.endpoint.getOutputCluster(ZclOnOffCluster.CLUSTER_ID);

    if (clientCluster == null) {
      log.error("[{}]: Error opening client cluster {} on {}", entityID, ZclOnOffCluster.CLUSTER_ID, this.endpoint);
    ...
    try {
      ...
      if (!bindResponse.isSuccess()) {
        log.error("[{}]: Error 0x{} setting client binding for cluster {}. {}", entityID,
            toHexString(bindResponse.getStatusCode()), ZclOnOffCluster.CLUSTER_ID, this.endpoint);
      }
    } catch (InterruptedException | ExecutionException e) {
      log.error("[{}]: Exception setting client binding to cluster {}: {}", entityID,
          ZclOnOffCluster.CLUSTER_ID, this.endpoint, e);
    }
  }
