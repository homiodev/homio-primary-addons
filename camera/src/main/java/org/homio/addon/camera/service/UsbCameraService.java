...
public class UsbCameraService extends BaseCameraService<UsbCameraEntity, UsbCameraService> {
  ...
  protected void pollCameraConnection() throws Exception {
    Set<String> aliveVideoDevices = context.media().video().getVideoDevices();
  ...
  protected void postInitializeCamera() {
    ...
    input = context.media().video().createVideoInputDevice(ieeeAddress);
    ...
    context.media().video().addVideoSource(getEntityID(), getUdpUrl());
  }
