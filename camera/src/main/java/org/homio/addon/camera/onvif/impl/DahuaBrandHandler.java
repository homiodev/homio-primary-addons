  ...
  private void processEvent(String content) {
    ...
    try {
      ...
      switch (code) {
        ...
        case "LensMaskOpen" -> getEndpointRequire(ENDPOINT_ENABLE_PRIVACY_MODE).setValue(OnOffType.ON);
        case "LensMaskClose" -> getEndpointRequire(ENDPOINT_ENABLE_PRIVACY_MODE).setValue(OnOffType.OFF);
        case "TimeChange", "NTPAdjustTime", "StorageChange", "Reboot", "NewFile", "VideoMotionInfo",
             "RtspSessionDisconnect", "LeFunctionStatusSync", "RecordDelete" -> {
        }
