...
public class Scratch3CameraBlocks extends Scratch3ExtensionBlocks {
    ...
    public Scratch3CameraBlocks(EntityContext entityContext, BroadcastLockManager broadcastLockManager,
        ...
        this.zoomReporter = withServerFfmpeg(Scratch3Block.ofEvaluate(50, "get_zoom", BlockType.reporter,
                "Get zoom of [VIDEO_STREAM]", this::getZoomReporter));

        this.tiltReporter = withServerFfmpeg(Scratch3Block.ofEvaluate(60, "get_tilt", BlockType.reporter,
                "Get tilt of [VIDEO_STREAM]", this::getTiltReporter));

        this.panReporter = withServerFfmpeg(Scratch3Block.ofEvaluate(70, "get_pan", BlockType.reporter,
                "Get pan of [VIDEO_STREAM]", this::getPanReporter));

        this.goToPresetReporter = withServerFfmpeg(Scratch3Block.ofEvaluate(80, "get_gotopreset", BlockType.reporter,
        ...
        this.goToPresetCommand = withServerFfmpeg(Scratch3Block.ofHandler(260, "to_to_preset",
    ...
    private <T extends BaseFFmpegStreamEntity> T getEntity(WorkspaceBlock workspaceBlock) {
        String entityID = workspaceBlock.getMenuValue(VIDEO_STREAM, this.ffmpegCameraMenu);
        BaseFFmpegStreamEntity entity = entityContext.getEntity(entityID);
        if (entity == null) {
            throw new RuntimeException("Video camera " + entityID + " not exists");
        }
        if (!entity.isStart()) {
    ...
    private Scratch3Block withServerOnvifAndValue(Scratch3Block scratch3Block, int value) {
        scratch3Block.addArgument(VALUE, value);
