...
import static org.touchhome.bundle.camera.onvif.util.IpCameraBindingConstants.CHANNEL_START_STREAM;
...
public abstract class BaseCameraStreamServerHandler<T extends BaseFFmpegCameraHandler> extends ChannelInboundHandlerAdapter {
    ...
    private void handleHttpContent(HttpContent msg) {
        ...
        if (msg instanceof LastHttpContent) {
            cameraHandler.bringCameraOnline();
            if (updateSnapshot) {
            ...
            switch (queryStringDecoder.path()) {
                case "/ipcamera.m3u8":
                    ...
                    if (!localFfmpeg.getIsAlive()) {
                        if (localFfmpeg.startConverting()) {
                            cameraHandler.setAttribute(CHANNEL_START_STREAM, OnOffType.ON);
                        }
                    } else {
                        localFfmpeg.setKeepAlive(8);
                        sendFile(ctx, httpRequest.uri(), "application/x-mpegurl", cameraHandler.getFfmpegHLSOutputPath());
                        return true;
                    }
                ...
                default:
                    if (httpRequest.uri().contains(".ts")) {
                        sendFile(ctx, queryStringDecoder.path(), "video/MP2T", cameraHandler.getFfmpegHLSOutputPath());
                    } else if (httpRequest.uri().contains(".gif")) {
                        sendFile(ctx, queryStringDecoder.path(), "image/gif", cameraHandler.getFfmpegGifOutputPath());
                    } else if (httpRequest.uri().contains(".jpg")) {
                        // Allow access to the preroll and postroll jpg files
                        sendFile(ctx, queryStringDecoder.path(), "image/jpg", cameraHandler.getFfmpegImageOutputPath());
                    } else if (httpRequest.uri().contains(".m4s") || httpRequest.uri().contains(".mp4")) {
                        sendFile(ctx, queryStringDecoder.path(), "video/mp4", cameraHandler.getFfmpegMP4OutputPath());
                    }
                    return true;
            }
