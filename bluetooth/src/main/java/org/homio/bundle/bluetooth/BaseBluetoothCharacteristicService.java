...
import org.homio.bundle.hquery.hardware.network.Network;
...
public abstract class BaseBluetoothCharacteristicService {
    ...
    protected void writeSafeValue(ThrowingRunnable<Exception> runnable) {
        if (hasAccess()) {
            try {
                ...
                lastError = "Error write: " + CommonUtils.getErrorMessage(ex);
    ...
    private String readSafeValue(Supplier<String> supplier) {
        try {
            ...
            lastError = "Error read: " + CommonUtils.getErrorMessage(ex);
