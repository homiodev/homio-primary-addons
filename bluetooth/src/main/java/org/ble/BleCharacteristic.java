...
class BleCharacteristic implements GattCharacteristic1, Properties {
  ...
  public void sendNotification() {
    try {
      ...
      PropertiesChangedSignal.PropertiesChanged signal = new PropertiesChangedSignal.PropertiesChanged(this.getPath().toString(), GATT_CHARACTERISTIC_INTERFACE, signalValue,
          new ArrayList<>());
  ...
  public byte[] ReadValue(Map<String, Variant> option) {
    ...
    if (readListener != null && System.currentTimeMillis() - lastReadTime >= minReadTimeout) {
      ...
      try {
        ...
        log.error("Error while read from ble: <{}>", CommonUtils.getErrorMessage(ex));
        value = new byte[0];
      }
    }
